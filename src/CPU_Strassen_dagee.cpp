...
void cpu_strassen_mul_dagee(const T *A, const T *B, T *C, size_t dim, BufMgr &bufMgr)
{
  if (dim == 1)
  {
    C[0] = A[0] * B[0];
  ...
  else
  {
    ...
    using DagExec = dagee::ATMIdagExecutor<CpuExec>;
    ...
    auto S_1Task = dag->addNode(cpuEx.makeTask(numThreads, addFunc, A_21, A_22, S_1, m));
    auto S_2Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, S_1, A_11, S_2, m));
    auto S_3Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, A_11, A_21, S_3, m));
    auto S_4Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, A_12, S_2, S_4, m));
    auto S_5Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, B_12, B_11, S_5, m));
    auto S_6Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, B_22, S_5, S_6, m));
    ...
    auto V_3Task = dag->addNode(cpuEx.makeTask(numThreads, addFunc, M_5, M_6, V_3, m));

    auto C_11Task = dag->addNode(cpuEx.makeTask(numThreads, addFunc, M_2, M_3, C_11, m));
    auto C_12Task = dag->addNode(cpuEx.makeTask(numThreads, addFunc, V_1, V_3, C_12, m));
    auto C_21Task = dag->addNode(cpuEx.makeTask(numThreads, subFunc, V_2, M_7, C_21, m));
    auto C_22Task = dag->addNode(cpuEx.makeTask(numThreads, addFunc, V_2, M_5, C_22, m));
...
#endif
  cpu_strassen_mul_dagee(A, B, C, n, bufMgr);
#if TIME == 1
